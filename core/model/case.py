...
class Case(Base):
    ...
    lifted_by_tag = Column(String)
    lifted_reason = Column(String)
    lifted_date = Column(DateTime)

    def __repr__(self):
        return f"<Case(case_id={self.case_id}, case_type={self.case_type} user_id={self.user_id}), mod_id={self.mod_id}, punishment={self.punishment}, reason={self.reason}>"
